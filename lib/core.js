...
var core = {
   ...
   signIn: function(username, password) {
	   ...
	   var postExpedition = function(feed) {
	      ...
	      } else if (feed.responseXML.getElementsByTagName('status')
   ...
   shuttle: function(url, content, postExpedition, feed) {
	   if (this instanceof arguments.callee) {
         ...
         let ctype = false;
	      if (feed.reqHeaders != undefined) {
		      for (var i = 0; i < feed.reqHeaders.length; i++) {
               if (feed.reqHeaders[i][0]=="content-type")
                  ctype=true;
         ...
         if (feed.content != undefined &&
             feed.content.__proto__ === new Object().__proto__) {
		      content = [];
	      ...
	      if (feed.upldOnProgress) {
		      this.__proto__.upload.onprogress = feed.upldOnProgress;
	      }
