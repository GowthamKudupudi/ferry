...
var core= {
    ...
    panelize:function(){
        if(this.panel && !this.panel.children[this.body.id]){
            ...
            if(this.panel.offsetWidth>innerWidth*0.9)this.panel.style.width=innerWidth*0.9+"px";
            if(this.panel.offsetHeight>innerHeight*0.9)this.panel.style.height=innerHeight*0.9+"px";
            this.panel.reAlign();
        }
    //return false;
    },
        ...
        switch(formMod){
            ...
            case('editProfileForm'):
                ...
                if(!core.userPic.exists){
                    ...
                    if(core.userPic.parentElement)core.userPic.parentElement.removeChild(core.userPic);
    ...
    customUploadFunction:function (uploadType){
        if(uploadType=="ProfPic"){
            ...
            if(!core.userPic.exists)document.getElementById('uploadBtnTxt').innerHTML='Change Pic';
        }
    },
    ...
    alignDoc:function(){
        ...
        if(window.search)search.reAlign();
    },
        ...
        switch(view){
            case ("home"):
                if(core.authenticated){
                    ...
                    if(document.getElementById('signInTool').children['username'].value=='')document.getElementById('signInTool').children['username'].focus();
                    else document.getElementById('signInTool').children['password'].focus();
                }
    ...
    objectize:function (){
        ...
        if(searchTool.children['passKeyBox'].offsetHeight<1){
            ...
            searchTool.children['searchMouth'].attributes['initValue'].value="Type-in Object Id";
            ...
            searchTool.children['searchMouth'].attributes['initValue'].value="Search";
            searchTool.children['searchMouth'].value="";
            searchTool.children['searchMouth'].onblur.call(searchTool.children['searchMouth']);
    ...
    isElmsClass:function(elm,className){
        for(i=0;i<elm.classList.length;i++){
            if(elm.classList[i]==className)return true;
        }
        return false;
    },
    shuttle: function(url,content,postExpedition,feed){
        if(this instanceof arguments.callee){
            if(feed.async==undefined)feed.async=true;
        ...
        }else throw 'xhr object constructor cannot be called as a function';
    },
    msgPanel:function(name,parent,objOwner,body,icon){
        ...
        if(name)title.innerHTML=name+'&nbsp;&nbsp;&nbsp;&nbsp';
        else title.innerHTML='&nbsp;&nbsp;&nbsp;&nbsp';
        ...
        panel.reAlign=function(){
            ...
            }else this.style.left=((window.innerWidth-this.body.offsetWidth)/2)+19*(msgPanels.length-1)+'px';
            ...
            if(!scriptExists){
                ...
                if(scripts.length>1){
                    ...
                    sc.onload()=function(){
                        ...
                        if(afterLoadScript)afterLoadScript();
                    }
                }
    ...
    selector:function(){
        ...
        if(elm.dblclick){
            ...
            if(!elm.selected){
                ...
                elm.allSelected=false;
                elm.selected=false;
                if(elm.style.border=='1px solid black')elm.style.border='1px solid yellow';
            }
        }
        elm.dblclick=false;
    },
    animatedImage:function(srcBnk,clickFunc,proto){
        ...
        this.trigMouseEvt=function(){
            ...
            this.onmouseup=function(){
                if(!this.disabled){
                    if(clickFunc)clickFunc.call(this);
    ...
    logStatus:function(){
        ...
        if(logPanel.offsetWidth>innerWidth*0.9)logPanel.style.width=innerWidth*0.9+"px";
        if(logPanel.offsetHeight>innerHeight*0.9)logPanel.style.height=innerHeight*0.9+"px";
        logPanel.reAlign();
    ...
    addContextMenu: function(elm,ecmenu){
        if(ecmenu.parentElement)ecmenu.parentElement.removeChild(ecmenu);
        elm.cmenu=ecmenu;
        cmenu.addItems=function(){
            ...
            if(event.srcElement==this)cmenu.mother=this;
            ...
            if(this==document.body || this.burstCMBubble){
                ...
                if(this.burstCMBubble)event.cancelBubble=true;
